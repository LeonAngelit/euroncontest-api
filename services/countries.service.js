...
class CountryService {
  ...
  async refresh(year) {
    ...
    url =
      process.env.BASE_URL +
      `${(year - process.env.FIRST_YEAR) * 10 + parseInt(process.env.FIRST_YEAR_SCRIPT)}.js`;
    if (resetNeeded) {
    ...
    for (const user of users) {
      let totalPoints = 0;
      const winnerId =
        user.winnerOption && user.winnerOption.length > 0
          ? user.winnerOption[0].countryId
          : null;
      const tailId =
        user.tailOption && user.tailOption.length > 0
          ? user.tailOption[0].countryId
          : null;
